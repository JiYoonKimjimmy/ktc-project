...
class TrafficExpireExecuteAdapterTest : BehaviorSpec({

    val reactiveStringRedisTemplate = EmbeddedRedis.reactiveStringRedisTemplate

    val trafficExpireExecuteAdapter = TrafficExpireExecuteAdapter(reactiveStringRedisTemplate)
    ...
    given("트래픽 만료 처리 요청되어") {
        ...
        `when`("59초 경과 후, 트래픽 마지막 polling 시간 '6초' 경과한 token 없는 경우") {
            trafficExpireExecuteAdapter.expireTraffic(now)

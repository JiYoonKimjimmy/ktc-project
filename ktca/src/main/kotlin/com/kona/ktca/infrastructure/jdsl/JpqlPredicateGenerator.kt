...
object JpqlPredicateGenerator {
    ...
    fun <T : Any, E: Any> whereEqualTo(column: KProperty1<E, T?>, value: T?): Predicatable? {
        return value?.let { Predicates.equal(value = Paths.path(column), compareValue = Expressions.value(it)) }
    }

    fun <T : Any, E: Any> whereEqualTo(column: Path<E>, value: T?): Predicatable? {
        return value?.let { Predicates.equal(value = column, compareValue = Expressions.value(it)) }
    }

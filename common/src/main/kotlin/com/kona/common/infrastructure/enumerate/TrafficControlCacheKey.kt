...
enum class TrafficControlCacheKey(
    ...
    BUCKET_REFILL_TIME(
        ...
        key = "ktc:{%s}:last_bucket_refill_time"
    ),
    THRESHOLD(
        note = "트래픽 분당 임계치 Key",
        key = "ktc:{%s}:threshold"
    ...
    companion object {
        fun getTrafficControlKeys(zoneId: String): Map<TrafficControlCacheKey, String> {
            return mapOf(
